...
const upazilas = {
  Dhaka: ["Dhanmondi", "Mirpur", "Gulshan", "Uttara", "Motijheel"],
  Chittagong: ["Panchlaish", "Halishahar", "Double Mooring", "Hathazari"],
  Rajshahi: ["Boalia", "Rajpari", "Matihar", "Shah Makhdum"],
  ...
  Mymensingh: ["Mymensingh Sadar", "Muktagachha", "Gafargaon", "Bhaluka"]
};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errData.message || "Failed to create request");
      }
    } catch (error) {
      console.error(error);
    ...
    return (
      <div className="max-w-2xl mx-auto mt-14 p-6 bg-red-50 border border-red-200 rounded-3xl flex flex-col items-center text-center shadow-md">
        ...
        <p className="text-gray-600 max-w-md">
          Admin has blocked your status. Blocked users are not allowed to create new blood donation requests.
        </p>
      </div>
    );
  }
      ...
      <div className="w-full max-w-[680px] bg-white rounded-3xl shadow-xl overflow-hidden mx-auto my-6">
        ...
        <div style={{ background: `linear-gradient(140deg, ${RED} 0%, ${RED_DARK} 100%)` }} className="px-6 pt-6 pb-1">
          <div className="relative flex items-center justify-center h-11">
            <Link href="/dashboard" className="absolute left-0 text-white active:scale-90 transition">
        ...
        <form onSubmit={handleSubmit} className="px-6 sm:px-8 pt-3 pb-8 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-[#F8FAFC] p-4 rounded-2xl border border-gray-100">
            <div>
              <label className="block text-[14px] font-semibold text-gray-500 mb-1.5">Requester Name</label>
              <input
                type="text"
                value={user?.name || ""}
                readOnly
            ...
            <div>
              <label className="block text-[14px] font-semibold text-gray-500 mb-1.5">Requester Email</label>
              <input
                type="email"
                value={user?.email || ""}
                readOnly
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Recipient Upazila *">
              <SelectBox
                ...
                placeholder={selectedDistrict ? "Select Upazila" : "Pick district first"}
                onChange={(v) => handleChange("recipientUpazila", v)}
                options={selectedDistrict ? upazilas[selectedDistrict] || [] : []}
          ...
          {/* Message */}
          <Field label="Request Message / Critical Details *">
            <textarea
              required
              disabled={submitting}
              rows={3}
