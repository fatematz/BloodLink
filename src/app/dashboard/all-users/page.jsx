  ...
  return (
    <>
      <tr className="hidden md:table-row hover:bg-gray-50/80 transition-colors border-b border-gray-100">
        <td className="p-5 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden ring-1 ring-gray-100 shrink-0">
            {user.image ? (
              ...
              <span className="text-gray-500 font-bold text-lg">{user.name?.charAt(0).toUpperCase()}</span>
            )}
          </div>
          <span className="font-bold text-gray-800 text-[14px]">{user.name}</span>
        </td>
        ...
        <td className="p-5 text-right relative"><ActionMenu user={user} actionWithId={actionWithId} /></td>
      </tr>
        ...
        <td colSpan={5} className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-gray-800 text-[14px]">{user.name}</p>
...
function RoleBadge({ role }) {
  return (
    <span className={`px-4 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider 
...
function StatusBadge({ status }) {
  return (
    <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase flex items-center gap-1.5 w-fit 
      ${status === 'active' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${status === 'active' ? 'bg-green-500' : 'bg-red-500'}`}></span>
      {status}
    </span>
  );
}
    ...
    <div className="group inline-block relative">
      ...
      <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[9999] overflow-hidden">
        <form action={actionWithId}>
          <button name="status" value={user.status === 'active' ? 'blocked' : 'active'}
          ...
          <button name="role" value="volunteer"
        ...
        <table className="w-full text-left border-collapse">
          <thead className="hidden md:table-header-group">
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="p-5 text-[12px] font-bold text-gray-400 uppercase">User</th>
              <th className="p-5 text-[12px] font-bold text-gray-400 uppercase">Email</th>
              <th className="p-5 text-[12px] font-bold text-gray-400 uppercase">Role</th>
              <th className="p-5 text-[12px] font-bold text-gray-400 uppercase">Status</th>
              <th className="p-5 text-[12px] font-bold text-gray-400 uppercase text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => (
