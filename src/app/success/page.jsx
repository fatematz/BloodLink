...
export default function SuccessPage() {
  ...
  useEffect(() => {
    if (!sessionId) { router.replace("/funding"); return; }
    ...
    const saveFund = async () => {
      try {
        ...
        const saveRes = await fetch("http://localhost:5000/api/funds", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
          },
          body: JSON.stringify({ name: userName, email: userEmail, amount: parseFloat(amount), transactionId: stripeTransactionId }),
        });
        ...
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center">
          {(status === "loading" || status === "saving") && (
            <>
              ...
              <p className="text-sm text-gray-400 mt-2">Please wait, we're saving your contribution.</p>
            </>
          )}
          {status === "done" && (
            <>
              <CheckCircle className="w-16 h-16 mx-auto mb-4" style={{ color: RED }} />
              <h2 className="text-2xl font-black text-gray-800">Thank You!</h2>
              <p className="text-gray-500 mt-2 text-sm">Your donation has been received and recorded successfully.</p>
          ...
          {status === "error" && (
            <>
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-red-50 flex items-center justify-center text-3xl">❌</div>
