...
export default function AllDonationRequest() {
  ...
  useEffect(() => {
    setLoading(true);
   fetch(`http://localhost:5000/api/donation-requests?status=pending&page=${page}&limit=5`, {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F4F6F9", fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif" }}>
      <Navbar />
      <div className="pb-16 pt-36 px-4 text-center" style={{ background: `linear-gradient(140deg, ${RED} 0%, ${RED_DARK} 100%)` }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        {loading ? (
          ...
          <p className="text-center text-gray-400 py-20">No pending donation requests found.</p>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {requests.map((req) => (
                <div key={req._id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col" style={{ border: "1px solid #F0F0F0" }}>
                  <div className="relative flex flex-col items-center pt-6 pb-8" style={{ background: `linear-gradient(140deg, ${RED}15 0%, ${RED}08 100%)` }}>
                    <span className="absolute top-3 left-3 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: `${RED}15`, color: RED }}>
                      <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: RED }}></span>
                      Pending
                    </span>
                    <div className="w-20 h-20 rounded-full flex items-center justify-center text-white font-black text-[26px] mt-4" style={{ background: `linear-gradient(135deg, ${RED} 0%, ${RED_DARK} 100%)`, boxShadow: `0 8px 24px -8px ${RED}80` }}>
                      {req.bloodGroup}
                    </div>
                  ...
                  <div className="px-5 pt-2 pb-5 flex flex-col gap-3 flex-1">
                    <div className="text-center mb-1">
                      <p className="font-bold text-gray-800 text-[17px]">{req.recipientName}</p>
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mt-0.5">Recipient</p>
                    </div>
                    <div className="h-px bg-gray-100" />
                    <div className="space-y-2.5">
                      <div className="flex items-start gap-2.5 text-[13px] text-gray-600">
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${RED}12` }}>
                          <MapPin size={13} style={{ color: RED }} />
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">Location</p>
                          <p className="font-medium text-gray-700">{req.recipientUpazila}, {req.recipientDistrict}</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2.5 text-[13px] text-gray-600">
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${RED}12` }}>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-700">{req.donationDate} | {req.donationTime}</p>
                        </div>
                      </div>
                    </div>
                    <button
                      ...
                      style={{ background: `linear-gradient(135deg, ${RED} 0%, ${RED_DARK} 100%)`, boxShadow: `0 6px 16px -6px ${RED}80` }}
                    >
                      View Details →
                    </button>
                  </div>
